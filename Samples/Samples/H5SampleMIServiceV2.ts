 ...
 */
 class H5SampleMIServiceV2 {
    ...
    private executeByRequest(): void {
        ...
        this.miService.executeRequestV2(myRequest).then(
            (response: IMIResponse) => {
                //Read results here
                for (let item of response.items[0]) {
    ...
    private executeByTransaction(): void {
        ...
        this.miService.executeV2(program, transaction, record, outputFields).then(
            (response: IMIResponse) => {
                //Read results here
    ...
    private executeWithOptionalArgs(): void {
        ...
        this.miService.executeRequestV2(myRequest).then(
            (response: IMIResponse) => {
                ...
                this.log.Info(`3: ${response.items[0].CONO} is a ${typeof response.items[0].CONO}`);
            }, (response: IMIResponse) => { 
    ...
    private executeMultiple(): void {
        ...
        const myPromise1 = this.miService.executeRequestV2(myRequest1);
        const myPromise2 = this.miService.executeRequestV2(myRequest2);
